...
<script type="text/javascript">
	$(function(){
		$("#btn_add").click(function(){
			location.href = "${pageContext.request.contextPath}/order?methods=addOrder&sum=${sum}&aid="+$("#address").val();
		})
	})
</script>
</head>
 				...
 				<tr>
 				 <td colspan="5">
 				 	...
 				 	<select id="address" style="width:60%" class="form-control">
 				 		<c:forEach items="${address}" var="a" varStatus="ai">
 				 			<option value="${a.a_id}">${a.a_name}&nbsp;&nbsp;${a.a_phone}&nbsp;&nbsp;${a.a_detail}</option>
 				 		</c:forEach>
 				 	</select>
 				 	<c:if test="${empty address}">
 				 		<a href="${pageContext.request.contextPath}/address?methods=selectAddr">添加收货地址</a>
 				 	</c:if>
 				 </td>
 				</tr>
			</table>
